...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get old unit number before update
    $old_unit_number = $tenant['unit_number'];
    
    // Update tenant information
    $update_sql = "UPDATE users SET full_name = ?, email = ?, phone = ?, emergency_contact = ?, 
                   unit_number = ?, unit_role = ?, is_primary_tenant = ?, is_verified = ?, updated_at = NOW() 
                   WHERE id = ?";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("ssssssiii", $full_name, $email, $phone, $emergency_contact, 
                           $unit_number, $unit_role, $is_primary_tenant, $is_verified, $tenant_id);
    
    if ($update_stmt->execute()) {
        $message = "Tenant updated successfully!";
        
        // Update register.php if unit was changed from vacant to occupied
        if (!empty($unit_number) && ($old_unit_number !== $unit_number || empty($old_unit_number))) {
            // Check if this unit was previously vacant in register
            $check_register_sql = "SELECT * FROM register WHERE unit_number = ? AND status = 'vacant'";
            $check_stmt = $conn->prepare($check_register_sql);
            $check_stmt->bind_param("s", $unit_number);
            $check_stmt->execute();
            $register_result = $check_stmt->get_result();
            
            if ($register_result->num_rows > 0) {
                // Update register to mark as occupied
                $update_register_sql = "UPDATE register SET status = 'occupied', tenant_name = ?, tenant_id = ?, updated_at = NOW() WHERE unit_number = ? AND status = 'vacant'";
                $update_register_stmt = $conn->prepare($update_register_sql);
                $update_register_stmt->bind_param("sis", $full_name, $tenant_id, $unit_number);
                $update_register_stmt->execute();
                $update_register_stmt->close();
                
                $message .= " Unit {$unit_number} marked as occupied in register.";
            }
            $check_stmt->close();
        }
        
        // If unit was removed, mark as vacant in register
        if (empty($unit_number) && !empty($old_unit_number)) {
            $update_register_sql = "UPDATE register SET status = 'vacant', tenant_name = NULL, tenant_id = NULL, updated_at = NOW() WHERE unit_number = ?";
            $update_register_stmt = $conn->prepare($update_register_sql);
            $update_register_stmt->bind_param("s", $old_unit_number);
            $update_register_stmt->execute();
            $update_register_stmt->close();
            
            $message .= " Unit {$old_unit_number} marked as vacant in register.";
        }
        ...
        $error = "Error updating tenant: " . $conn->error;
    }
    $update_stmt->close();
}
?>
...
<head>
    ...
    <title>Edit Tenant - <?= htmlspecialchars($tenant['full_name']) ?></title>
...
<body>
        ...
        <div class="card">
            <div class="tenant-info">
                ...
                <div>
                    ...
                    <p>Tenant ID: <?= $tenant['id'] ?> | Created: <?= date('M j, Y', strtotime($tenant['created_at'])) ?></p>
                </div>
            </div>
            
            <?php if (!empty($tenant['unit_number'])): ?>
                <div class="current-unit">
                    <h4><i class="fas fa-info-circle"></i> Current Unit Assignment</h4>
            ...
            <form method="POST" action="">
                ...
                <div class="form-section">
                    <h3 class="section-title"><i class="fas fa-home"></i> Unit Information</h3>
                    
                    <div class="form-group">
                        <label for="unit_number" class="form-label">Unit Number</label>
                        <select name="unit_number" id="unit_number" class="form-select">
                        ...
                        <small style="color: #7f8c8d; margin-top: 5px; display: block;">
                            Changing unit assignment will update the register automatically
                        </small>
                    </div>
                    
                    <div class="form-group">
                        <label for="unit_role" class="form-label">Unit Role</label>
                        <select name="unit_role" id="unit_role" class="form-select">
                    ...
                    <div class="checkbox-group">
                        ...
                        <label for="is_primary_tenant">Primary Tenant</label>
                    </div>
                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="checkbox-group">
                        <input type="checkbox" name="is_verified" id="is_verified" value="1" 
                            <?= ($tenant['is_verified'] ?? 0) ? 'checked' : '' ?>>
                        <label for="is_verified">Account Verified</label>
                    </div>
                    <small style="color: #7f8c8d; margin-top: -10px; display: block; margin-bottom: 20px;">
                        Verified tenants can access all tenant portal features
                    </small>
                </div>
                ...
                <div class="action-buttons">
                    ...
                    <a href="manage_tenants.php" class="btn btn-primary">
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            // Optional phone validation for Kenya format
            if (phone && !phone.match(/^\+254[17]\d{8}$/)) {
                if (!confirm('Phone number may not be in correct Kenyan format (+254XXXXXXXXX). Continue anyway?')) {
                    e.preventDefault();
                    return;
                }
            }
            
            // Confirm unit assignment changes
            const currentUnit = '<?= $tenant['unit_number'] ?? '' ?>';
            const newUnit = document.getElementById('unit_number').value;
            
            if (currentUnit !== newUnit) {
                if (newUnit && !confirm(Changing unit from "${currentUnit || 'Not assigned'}" to "${newUnit}". This will update the register. Continue?)) {
                    e.preventDefault();
                    return;
                }
                if (!newUnit && currentUnit) {
                    if (!confirm(Removing unit assignment "${currentUnit}". This will mark the unit as vacant in register. Continue?)) {
                        e.preventDefault();
                        return;
                    }
                }
